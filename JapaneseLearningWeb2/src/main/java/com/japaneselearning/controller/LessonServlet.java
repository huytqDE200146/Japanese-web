...
public class LessonServlet extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        // 2. Lấy danh sách ID các bài học đã hoàn thành (nếu đã đăng nhập)
        List<Integer> completedIds = new ArrayList<>();
        if (user != null) {
            // Bạn cần đảm bảo đã thêm hàm getCompletedLessonIds vào LessonDAO
            completedIds = lessonDAO.getCompletedLessonIds(user.getId());
        }
        request.setAttribute("completedIds", completedIds);

        // 3. Lấy danh sách bài học đã được nhóm theo Category
        ...
        // 4. Lấy danh sách phẳng (flat list) để tương thích ngược
