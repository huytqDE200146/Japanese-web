        ...
        <div class="lesson-main">
            ...
            <div class="lesson-hero animate-fade-in">
                ...
                <div class="lesson-meta">
                    ...
                    <span class="meta-badge">⏱️ ~30 phút</span>
                </div>
            </div>
...
=======
    ...
    <body>
        ...
        <style>
            ...
            /* Make jp-word cells flex for alignment */
            .lesson-content-area td.jp-word,
            .lesson-content-area .japanese {
                position: relative;
            }
        </style>
        ...
        // ===== Web Speech API - Japanese TTS =====
            document.addEventListener('DOMContentLoaded', function () {
                // Japanese character ranges (Hiragana, Katakana, Kanji, special marks)
                const jpRegex = /[\u3000-\u303F\u3040-\u309F\u30A0-\u30FF\u4E00-\u9FFF\uFF00-\uFFEF\u3400-\u4DBF]+/g;

                // Find all elements with Japanese text
                const selectors = '.lesson-content-area .jp-word, .lesson-content-area .japanese, .lesson-content-area .grammar-structure';
                const jpElements = document.querySelectorAll(selectors);

                jpElements.forEach(function (el) {
                    let text = el.textContent.trim();
                    // Remove text inside parentheses (full-width and half-width)
                    text = text.replace(/（[^）]*）/g, '').replace(/\([^)]*\)/g, '');
                    // Check if element contains Japanese characters
                    const jpMatches = text.match(jpRegex);
                    if (!jpMatches || jpMatches.length === 0)
                        return;

                    // Extract all Japanese text for speech
                    const jpText = jpMatches.join(' ');
                    ...
                    btn.textContent = '🔊';
                    btn.setAttribute('data-jp', jpText);

                    btn.addEventListener('click', function (e) {
                        e.preventDefault();
                        e.stopPropagation();
                        speakJapanese(jpText, btn);
                    });
...
<<<<<<< HEAD
        <p style="color:var(--muted-text);margin-bottom:1.5rem;">AI sẽ tạo quiz từ nội dung bài "<%= lesson != null ? lesson.getDescription() : "" %>"</p>
=======
...
>>>>>>> f88f49bbc623c4dcecf2fbf29b3238f8f6b4161b
    ...
    <!-- Results -->
<<<<<<< HEAD
    <div id="aiQuizResult" style="display:none;" class="sidebar-card" style="padding:3rem;text-align:center;">
=======
...
<<<<<<< HEAD
    // Get lesson content from the page
=======
>>>>>>> f88f49bbc623c4dcecf2fbf29b3238f8f6b4161b
...
<div class="completion-zone mt-5 text-center p-4" style="background: rgba(255,255,255,0.05); border-radius: 16px;">
    <div id="timerDisplay" class="mb-3">
        <span class="badge bg-warning text-dark" style="font-size: 1.1rem;">
            ⏱️ Thời gian học còn lại: <span id="countdown">30:00</span>
        </span>
    </div>
    
    <form action="complete-lesson" method="POST" id="completeForm">
        <input type="hidden" name="lessonId" value="<%= lesson.getLessonId() %>">
        <button type="submit" id="btnComplete" class="btn btn-success px-5 py-2" disabled 
                style="opacity: 0.5; cursor: not-allowed; font-weight: 600;">
            🎉 Hoàn thành
        </button>
    </form>
</div>
...
<script>
    let timeLeft = 1; // 30 phút tính bằng giây
    ...
    const timer = setInterval(() => {
        ...
        if (timeLeft <= 0) {
            clearInterval(timer);
            countdownEl.innerHTML = "0:00 - Bạn đã đủ điều kiện hoàn thành!";
