...
document.addEventListener('DOMContentLoaded', function () {
    ...
    jpElements.forEach(function (el) {
        ...
        if (!jpMatches || jpMatches.length === 0) return;
...
function speakJapanese(text, btn) {
    ...
    const jaVoice = voices.find(function (v) { return v.lang.startsWith('ja'); });
    if (jaVoice) utterance.voice = jaVoice;

    utterance.onstart = function () { if (btn) btn.classList.add('speaking'); };
...
let timeLeft = 1; // Thời gian đếm ngược
const countdownEl = document.getElementById('countdown');
const btnComplete = document.getElementById('btnComplete');

if (countdownEl && btnComplete) {
